        ...
        <div class="row">
            <div class="panel panel-default">
                <div class="panel-body">
                    <form class="form-inline" role="form" method="get" action="/manager/analytics/members">
...
<?php foreach ($countries as $countryCode => $countryName): ?>
                    <td class="text-nowrap text-right"><?php echo h(number_format($members[$date][$countryCode])); ?></td>
<?php endforeach; ?>
                </tr>
<?php endforeach; ?>
                <tr>
...
<?php foreach ($countries as $countryCode => $countryName): ?>
                    <td class="text-nowrap text-right"><?php echo h(number_format($members['summary'][$countryCode])); ?></td>
<?php endforeach; ?>
                </tr>
            </table>
        </div>
...
<script type="text/javascript">
    ...
    function drawChart() {
        ...
        dataCal.addColumn({type: 'number', id: 'Members'});

        dataCal.addRows([
<?php foreach ($dates as $date): ?>
            [new Date('<?php echo h($date); ?>'), <?php echo h($members[$date][0]); ?>],
<?php endforeach; ?>
        ]);
        ...
        var dataGeo = new google.visualization.arrayToDataTable([
            ['Country', 'Members'],
<?php foreach ($countries as $countryCode => $countryName): ?>
<?php if ($members['summary'][$countryCode] > 0): ?>
            ['<?php echo addslashes($countryName); ?>', <?php echo $members['summary'][$countryCode]; ?>],
<?php endif; ?>
<?php endforeach; ?>
        ]);
